...
import { CheckCircle, Search, Eye, GraduationCap, Building2, Star } from 'lucide-react';
...
const TrainingCompletionPage: React.FC = () => {
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        const rows = applications
          .filter(app => app.status === 'TRAINING_COMPLETED')
          .map(app => ({
            application: app,
        ...
        console.error('Failed to fetch training completion data', err);
      } finally {
        setLoading(false);
      }
    };
    ...
    return (
      <div className="flex h-full items-center justify-center">
        <div className="flex flex-col items-center gap-4">
          ...
          <p className="text-slate-500 font-medium animate-pulse">Loading training completions...</p>
        </div>
      </div>
    );
  }
    ...
    >
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-slate-500 mt-1">Students who have completed their training period.</p>
        </div>
      </div>
      ...
      <div className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
        {filteredData.length === 0 ? (
          <div className="p-12 text-center flex flex-col items-center justify-center">
            ...
            <h3 className="text-lg font-medium text-slate-900">No completed trainings</h3>
            <p className="text-slate-500 mt-1">
              {searchTerm
                ? "Adjust your search to find what you're looking for."
                : 'No students have completed training yet.'}
            </p>
          </div>
        ) : (
                  ...
                  >
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-3">
                        <div className="w-10 h-10 rounded-full bg-emerald-100 border border-emerald-200 flex items-center justify-center overflow-hidden flex-shrink-0">
                          {row.student.avatar ? (
...
export default TrainingCompletionPage;
